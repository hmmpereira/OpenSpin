    ...
    NumberFormat formatter = new DecimalFormat("#.0000");
        try{
           if (Rotationdevice.equals("Motor_1") || Rotationdevice.equals("Motor_2") || Rotationdevice.equals("Motor_3") || Rotationdevice.equals("Motor_4")){
               ...
               mmc_.setProperty("SRotation", "Command", "MOTOR0MOVESTEPS="+Step_f+";");
               
            }
            else{
                try {
                    ////
                    double position = mmc_.getPosition(Rotationdevice);
                    double STEP=Double.parseDouble(Step);
                    double position_new=position-STEP;
                    mmc_.setPosition(Rotationdevice,position_new);
                        String Position_new=formatter.format(mmc_.getPosition(Rotationdevice));  
                } catch (Exception ex) {
